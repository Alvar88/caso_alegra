...
const icpColors: Record<string, string> = {
  HOT: '#EF4444', WARM: '#B45309', COLD: '#9CA3AF',
}
const icpBg: Record<string, string> = {
  HOT: 'rgba(239,68,68,0.08)', WARM: '#FFF8E7', COLD: '#F5F4FA',
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      const [d, r, c, co] = await Promise.all([
      ...
      setDeals(d.data ?? [])
      setReps(r.data ?? [])
      setTotalContacts(c.count ?? 0)
      setTotalCompanies(co.count ?? 0)
      setLoading(false)
    }
    load()
  }, [])
  ...
  const pipelineValue = activeDeals.reduce((s, d) => s + Number(d.mrr), 0)
  const hotDeals = activeDeals.filter(d => d.icp_score === 'HOT').length
  const warmDeals = activeDeals.filter(d => d.icp_score === 'WARM').length
  const coldDeals = activeDeals.filter(d => d.icp_score === 'COLD').length
  const withTime = deals.filter(d => d.response_time_min)
  const avgResponse = withTime.length
    ? Math.round(withTime.reduce((s, d) => s + d.response_time_min, 0) / withTime.length) : 0
    ...
    <div style={{ padding: '32px 36px', maxWidth: 1200 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 16 }}>
        {[
          ...
          <div key={kpi.label} style={{
            ...
            boxShadow: '0 2px 12px rgba(0,192,115,0.08)',
          }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
        {[
          ...
          { label: 'Total Contactos', value: totalContacts, sub: `${totalCompanies} empresas`, color: '#1A1A2E', bg: '#FFFFFF', border: '#E5E7EB' },
        ].map(kpi => (
          <div key={kpi.label} style={{
            ...
            <div style={{
              ...
              fontSize: 22, fontWeight: 800, color: kpi.color,
            }}>{kpi.value}</div>
            <div>
          ...
          {reps.map((rep: any, i: number) => (
            <div key={rep.id} style={{ padding: '16px 22px', borderBottom: i < reps.length - 1 ? '1px solid #E5E7EB' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                <div style={{
                  width: 38, height: 38, borderRadius: '50%', background: '#00C073',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{
                    ...
                    color: rep.capacity_score >= 80 ? '#EF4444' : rep.capacity_score >= 60 ? '#B45309' : '#00A363',
              ...
              <div style={{ height: 5, borderRadius: 3, background: '#F5F4FA', overflow: 'hidden' }}>
                <div style={{
                  height: '100%', width: `${rep.capacity_score}%`, borderRadius: 3,
                  background: rep.capacity_score >= 80 ? '#EF4444' : rep.capacity_score >= 60 ? '#F59E0B' : '#00C073',
                }} />
              </div>
            </div>
          ))}
      ...
      <div style={{
        ...
        background: '#E8F8F0', border: '1px solid #A7F3D0', borderRadius: 14,
        ...
        <Link href="/agente" style={{
          padding: '12px 24px', background: '#00C073', color: '#fff',
