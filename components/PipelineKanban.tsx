...
export default function PipelineKanban() {
  ...
  useEffect(() => {
    fetch('/api/db?type=pipeline').then(r => r.json()).then(({ deals, companies }) => {
      setDeals(deals ?? [])
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ color: '#9CA3AF', fontSize: 13 }}>Cargando pipeline…</div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 28 }}>
        {(['contadores', 'pymes'] as const).map(p => (
          <button key={p} onClick={() => setActivePipeline(p)} style={{
            ...
            background: activePipeline === p ? '#00C073' : '#fff',
            ...
            <div key={stage.id} style={{ minWidth: 220, maxWidth: 240, flexShrink: 0 }}>
              <div style={{ padding: '10px 14px', borderRadius: '10px 10px 0 0', background: '#F5F4FA', border: '1px solid #E5E7EB', borderBottom: 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, background: '#E5E7EB', borderRadius: 10, padding: '1px 8px', color: '#6B7280' }}>{stageDeals.length}</span>
                </div>
              ...
              <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: '0 0 10px 10px', minHeight: 60, padding: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {stageDeals.map(deal => {
                  ...
                  return (
                    <div key={deal.id} style={{ background: '#FAFAFA', border: '1px solid #E5E7EB', borderLeft: `3px solid ${icpColor[deal.icp_score]}`, borderRadius: 8, padding: '10px 12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7 }}>
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, color: icpColor[deal.icp_score], background: icpBg[deal.icp_score] }}>{deal.icp_score}</span>
                        {Number(deal.mrr) > 0
                          ? <span style={{ fontSize: 11, fontWeight: 700, color: '#1A1A2E' }}>${Number(deal.mrr).toLocaleString()}/m</span>
                          : deal.loss_reason && <span style={{ fontSize: 10, color: '#EF4444' }}>{deal.loss_reason}</span>
                        }
                      </div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#1A1A2E', marginBottom: 3 }}>{deal.contact?.first_name} {deal.contact?.last_name}</div>
                      {companyName
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 10, color: '#9CA3AF', background: '#F5F4FA', padding: '1px 7px', borderRadius: 4, fontWeight: 600 }}>{deal.rep?.avatar}</span>
                      </div>
                ...
                {stageDeals.length === 0 && <div style={{ textAlign: 'center', color: '#E5E7EB', fontSize: 13, padding: '18px 0' }}>—</div>}
              </div>
            </div>
          )
        })}
